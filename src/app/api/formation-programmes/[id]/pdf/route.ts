...
<body>
    <div class="header">
        ...
        <div class="code">Code: ${formation.code_formation}</div>
    </div>
    ...
    <div class="section">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                <strong>Durée:</strong> ${formation.modalites_acces?.duree || 'Non spécifiée'}
            </div>
            <div class="info-item">
                <strong>Tarif:</strong> ${formation.modalites_acces?.tarif || 'Non spécifié'}€
            </div>
            <div class="info-item">
                <strong>Horaires:</strong> ${formation.modalites_acces?.horaires || 'Non spécifiés'}
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        ${formation.programme_detail.map((jour: any) => `
        <div class="programme-detail">
            <div class="jour">${jour.jour} - ${jour.duree}</div>
            ${jour.modules ? jour.modules.map((module: any) => `
            <div class="module">
    ...
    ${formation.modalites_acces?.prerequis ? `
    <div class="section">
        <h2>Prérequis</h2>
        <p>${formation.modalites_acces.prerequis}</p>
    </div>
    ` : ''}

    ${formation.modalites_acces?.public_concerne ? `
    <div class="section">
        <h2>Public concerné</h2>
        <p>${formation.modalites_acces.public_concerne}</p>
    </div>
    ` : ''}
    ...
    <div class="section">
        ...
        <div class="programme-detail">
            ${formatRichText(formation.modalites_pedagogiques)}
        </div>
    </div>
    ` : ''}
    ...
    <div class="section">
        ...
        <ul>
            ${formation.ressources_dispo.map((ressource: any) => `
    ...
    <div class="section">
        ...
        <p><strong>Plateforme d'évaluation:</strong> ${formation.modalites_evaluation.plateforme_evaluation}</p>
        ` : ''}
        ${formation.modalites_evaluation.grille_analyse ? `
        <p><strong>Grille d'analyse:</strong> ${formation.modalites_evaluation.grille_analyse}</p>
        ` : ''}
    </div>
    ` : ''}
    ...
    <div class="section">
        ...
        <div class="contact-info">
            <h3>${formation.contact_formateur.nom}</h3>
            ${formation.contact_formateur.role ? `<p><strong>Rôle:</strong> ${formation.contact_formateur.role}</p>` : ''}
            ${formation.contact_formateur.email ? `<p><strong>Email:</strong> ${formation.contact_formateur.email}</p>` : ''}
            ${formation.contact_formateur.telephone ? `<p><strong>Téléphone:</strong> ${formation.contact_formateur.telephone}</p>` : ''}
            ${formation.contact_formateur.biographie ? `<p><strong>Biographie:</strong> ${formation.contact_formateur.biographie}</p>` : ''}
        </div>
    </div>
    ` : ''}
