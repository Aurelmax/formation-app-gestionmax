  ...
  try {
    ...
    for (const programme of programmes) {
      ...
      if (!programme['modalitesEvaluation']) {
        updateData['modalitesEvaluation'] =
          'Évaluation des connaissances théoriques et travaux pratiques'
      }
      ...
      if (!programme['accessibiliteHandicap']) {
        updateData['accessibiliteHandicap'] =
      ...
      if (!programme['formateurBiographie']) {
        updateData['formateurBiographie'] =
      ...
      if (Object.keys(updateData).length > 0) {
        await collection.updateOne({ _id: programme._id }, { $set: updateData })
