  ...
  const handleSave = async (status: 'brouillon' | 'publie') => {
    ...
    try {
      const articleData = {
        ...formData,
        statut: status,
        datePublication: status === 'publie' ? new Date().toISOString().split('T')[0] : formData.datePublication || new Date().toISOString().split('T')[0]
      };

      if (mode === 'create') {
        await BlogService.createArticle(articleData);
        toast.success(`Article ${status === 'publie' ? 'publié' : 'sauvegardé en brouillon'} avec succès`);
      } else if (articleId) {
