...
export function RendezVousManagement() {
  ...
  const loadRendezVous = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`/api/rendez-vous?${new URLSearchParams(filters as any)}`);
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Gérez tous les rendez-vous de formation
          </p>
        </div>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Select
              ...
              onValueChange={(value) => setFilters({ ...filters, statut: value === 'all' ? undefined : value })}
            >
              <SelectTrigger>
            ...
            <Select
              ...
              onValueChange={(value) => setFilters({ ...filters, lieu: value === 'all' ? undefined : value })}
            >
              <SelectTrigger>
                  ...
                  <TableCell className="text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuItem onClick={() => router.push(`/admin/rendez-vous/${rdv.id}`)}>
                          <Eye className="h-4 w-4 mr-2" />
                          Voir
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => router.push(`/admin/rendez-vous/${rdv.id}/edit`)}>
                        ...
                        {rdv.statut === 'en_attente' && (
                          <DropdownMenuItem onClick={() => handleStatusChange(rdv.id, 'confirme')}>
