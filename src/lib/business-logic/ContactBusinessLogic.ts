...
export class ContactBusinessLogic {
  ...
  static calculateMessageStats(messages: ContactMessage[]) {
    ...
    const byStatut = messages.reduce((acc, message) => {
    ...
    const byPriorite = messages.reduce((acc, message) => {
      acc[message.priorite] = (acc[message.priorite] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);

    const nonTraites = messages.filter(m => m.statut === 'nouveau' || m.statut === 'en_cours').length;
