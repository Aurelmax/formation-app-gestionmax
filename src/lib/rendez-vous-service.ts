import { RendezVous, CreateRendezVousRequest, UpdateRendezVousRequest, RendezVousFilters, RendezVousStats } from '@/types/rendez-vous';
...
class RendezVousService {
  ...
  private saveToStorage() {
    try {
      if (typeof window !== 'undefined') {
        const customRendezVous = this.rendezVous.filter(rdv => 
          !MOCK_RENDEZ_VOUS.some(mock => mock.id === rdv.id)
        );
  ...
  async getRendezVous(filters?: RendezVousFilters): Promise<{ rendezVous: RendezVous[]; total: number; stats: RendezVousStats }> {
    await delay();
  ...
  async createRendezVous(data: CreateRendezVousRequest): Promise<RendezVous> {
    ...
    const newRendezVous: RendezVous = {
      id: `rdv_${Date.now()}`,
      programmeTitre: 'Programme de formation', // À récupérer depuis le service programmes
      statut: 'en_attente',
      duree: data.duree || 30,
      rappelEnvoye: false,
